...
namespace DataAccess
{
    public class SqlCommandExecutor
    {
        ...
        public SqlCommandExecutor(string server, string database)
           : this($"Server={server}; Database={database}; Trusted_Connection=True;")
        {
        }
        ...
        public void ExecuteNonQuery(DataDelegate dataDelegate)
        {
        ...
        public T ExecuteNonQuery<T>(NonQueryDataDelegate<T> dataDelegate)
        {
            using (var transaction = new TransactionScope())
            {
                using (var connection = new SqlConnection(connectionString))
        ...
        public T ExecuteReader<T>(IDataReaderDelegate<T> dataDelegate)
        {
            using (var connection = new SqlConnection(connectionString))
            {
                using (var command = new SqlCommand(dataDelegate.ProcedureName, connection))
                {
                    ...
                    return dataDelegate.Translate(command, reader);
                }
            }
        }
    }
